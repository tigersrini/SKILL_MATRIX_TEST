...
with tab1:
    ...
    if radar_trigger:
        ...
        for idx, row in members.iterrows():
            ...
            # Calculate self-rating average from Excel data
            self_ratings = [rating_to_num(row.get(skill, "")) for skill in core_skills]
            avg_self = round(sum(self_ratings)/len(self_ratings), 2) if self_ratings else 0
            self_avgs.append(avg_self)
            
        member_names += [member_names[0]]
        mgr_avgs += [mgr_avgs[0]]
        self_avgs += [self_avgs[0]]
        fig_member_strength = go.Figure()
        fig_member_strength.add_trace(go.Scatterpolar(
            r=self_avgs,
            theta=member_names,
            name='Self Avg Rating',
            line=dict(color='royalblue', width=4),
            mode="lines+markers+text",
            text=[str(v) for v in self_avgs],
    ...
    # --- Team Members Strength Radar (Both Self and Manager Ratings) ---
    if selected_team and selected_team in TEAM_BLOCKS:
        ...
        fig_member_strength.update_layout(
            ...
            showlegend=True,
            title=dict(text=f"{selected_team} Members Strength Radar", font=dict(size=20)),
            width=700,
            height=600,
            margin=dict(l=60, r=60, t=60, b=60),
            transition={'duration': 1000}
        ...
        members_avg_mgr = round(sum(mgr_avgs[:-1]) / len(mgr_avgs[:-1]), 2) if len(mgr_avgs) > 1 else 0
        members_avg_self = round(sum(self_avgs[:-1]) / len(self_avgs[:-1]), 2) if len(self_avgs) > 1 else 0
        st.markdown(f"<span style='font-size:18px; color:gray;'>Team Self Avg: <b>{members_avg_self}/3</b> | Manager Avg: <b>{members_avg_mgr}/3</b></span>", unsafe_allow_html=True)
        st.plotly_chart(fig_member_strength, use_container_width=True)
